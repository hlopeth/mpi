...
std::vector<double> Matrix::getRow(uint32_t row) const
{
	std::vector<double> data;
	for (int i = 0; i < _colls; i++)
	{
		data.push_back(this->operator()(row, i));
	}

	return data;
}

std::vector<double> Matrix::getColl(uint32_t coll) const
{
	std::vector<double> data;
	for (int i = 0; i < _rows; i++)
	{
		data.push_back(this->operator()(i, coll));
	}
	return data;
}
